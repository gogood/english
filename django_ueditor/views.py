...
def UploadFile(request,uploadtype,uploadpath):
    if not request.method=="POST": return  JsonResponse(dict(state=u'ERROR'),safe=False)
    state="SUCCESS"
    file=request.FILES.get("upfile",None)
    #如果没有提交upfile则返回错误
    if file is None:return  JsonResponse(dict(state=u'ERROR'),safe=False)
    #取得上传的文件的原始名称
    ...
    if not original_ext  in allow_type and allow_type != u'*':
    ...
    if not USettings.sae:
        ...
        """sae环境
        """
        fileName= GenerateRndFilename(file.name)
        import sae.const   
        ...
        OutputFile = s.put(USettings.domain_name, fileName, ob)   
    ...
    if uploadtype=="scrawlbg":#上传涂鸦背景
        ...
        return JsonResponse(rInfo)

#图片文件管理器
@csrf_exempt
def ImageManager(request,imagepath):
    if not request.method!="GET": 
        return  JsonResponse(dict(state=u'ERROR'))
    #取得动作
    ...
    if action=="get":
        if not USettings.sae:
            ...
            return JsonResponse(ReadDirImageFiles(TargetPath), safe=False)
        else:
            return JsonResponse(ReadStorageImageFiles(''),safe=False)
#遍历所有文件清单
...
def ReadStorageImageFiles(path):
    import sae.const   
    from sae.storage import Connection
    access_key = sae.const.ACCESS_KEY   
    secret_key = sae.const.SECRET_KEY   
    appname = sae.const.APP_NAME   
    conn = sae.storage.Connection(accesskey=access_key, secretkey=secret_key, account=appname, retries=3)
    bucket = conn.get_bucket(USettings.domain_name)
    files = ""
...
def RemoteCatchImage(request,imagepath):
    ...
    if upfile_url is None:
        return JsonResponse(dict(state=u'ERROR'),safe=False)
    import urllib
    ...
    try:
        ...
        return JsonResponse(dict(tip=unicode(tip)),safe=False)
    #取得目标抓取的文件名称
    ...
    try:
        if not USettings.sae:
            ...
            """sae环境
            """
            fileName= GenerateRndFilename('network.jpg')
            import sae.const   
            access_key = sae.const.ACCESS_KEY   
            secret_key = sae.const.SECRET_KEY   
            appname = sae.const.APP_NAME   
            import sae.storage   
            s = sae.storage.Client()   
            ob = sae.storage.Object(CatchFile.read())
            OutFile = s.put(USettings.domain_name, fileName, ob)   
        rInfo={
                'url'   : OutFile,                      # 新地址一ue_separate_ue新地址二ue_separate_ue新地址三',
                'srcUrl':upfile_url,                    #原始地址一ue_separate_ue原始地址二ue_separate_ue原始地址三',
                'tip'   :u'远程图片抓取成功！'           #'状态提示'
            }
        return JsonResponse(rInfo,safe=False)
    except Exception,E:
    ...
    if action=="tmpImg":
        ...
        try:
            ...
            if not USettings.sae:
                ...
                import sae.storage   
                s = sae.storage.Client()   
                ob = sae.storage.Object(base64.decodestring(content))
                OutputFile = s.put(USettings.domain_name, OutputFile, ob)   
                state="SUCCESS"
        except Exception,E:
            state="ERROR:"
        rInfo={
